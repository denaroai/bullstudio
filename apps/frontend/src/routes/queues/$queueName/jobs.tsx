...
function QueueJobsPage() {
  ...
  const jobsListScrollKey = useMemo(
    ...
    [queueParam, statusFilter, q, page, pageSize, sortField, sortOrder],
  );
  ...
  // Keep expanded row in sync with URL so browser Back restores list filters + scroll.
  useEffect(() => {
    setExpandedJobId(selected ?? null);
  }, [selected]);

  // Re-run when job count changes so scroll clamp uses the final table height after render.
  ...
  useLayoutEffect(() => {
    ...
    if (y == null || !container) return;

    let cancelled = false;
    const apply = () => {
      if (cancelled || !tableScrollRef.current) return;
      const max = Math.max(
        0,
        tableScrollRef.current.scrollHeight - tableScrollRef.current.clientHeight,
      );
      tableScrollRef.current.scrollTop = Math.min(y, max);
    };
